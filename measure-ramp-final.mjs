...
async function measureRampFinal() {
  ...
  const results = await page.evaluate(() => {
    const find = (targetText) => {
      const elements = [...document.querySelectorAll('*')]
        .filter(e => e.children.length === 0 &&
                     e.textContent.trim().toLowerCase() === targetText.toLowerCase());
    ...
    const hero = h1s.length > 0 ? {
      tag: 'h1 (split across 3 elements)',
    ...
    return {
      ...
      body: find('We believe marketing doesn\'t have to be complicated. That\'s why we\'ve built a cr'),
  ...
  console.log('\n════════════════════════════════════════════════════════');
  console.log('RAMP STUDIO — CORRECTED MEASUREMENTS (375×812 MOBILE)');
  console.log('════════════════════════════════════════════════════════\n');
  ...
  report('EYEBROW / SECTION LABEL', results.eyebrow);
  report('BODY PARAGRAPH', results.body);
  report('CTA BUTTON — View Course', results.ctaViewCourse);
  report('CTA BUTTON — View All', results.ctaViewAll);
  report('CARD TITLE — Social Media Growth', results.cardTitle);
